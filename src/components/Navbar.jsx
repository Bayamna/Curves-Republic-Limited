...
const Navbar = () => {
  ...
  return (
    <div>
        <nav className='bg-gradient-to-r z-999 from-brown to-lightBrown text-white flex py-7 px-10 justify-between items-center text-lg fixed top-0 w-screen border-b-2 border-lightGold '>
            ...
            {!open ? <GiHamburgerMenu className='text-2xl lg:hidden cursor-pointer hover:text-gold' onClick={()=>setOpen(true)}/>:<IoMdClose className='text-3xl lg:hidden cursor-pointer hover:text-gold' onClick={()=>setOpen(false)}/>}
        </nav>
        {open ?  <div className='flex-row gap-6 bg-brown h-screen w-screen fixed p-14 text-lg top-0 text-white text-center mt-[4rem] z-998 lg:hidden'>
            ...
            <button className='text-[16px] text-brown bg-white font-bold px-[24px] py-[9px] cursor-pointer rounded-lg mt-6'>
                <a href='/booking'>Book Us Today!</a>
            </button>
        </div>: null}
